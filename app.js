...
// Alias nomi club: normalizza varianti CSV → nome canonico (da popolare con lista UCL definitiva)
const CLUB_ALIASES = {};
...
const SUPERADMIN_UID="s7PTpp7BExQAYQCha3sL7eYiamI2";
let superadminUnlocked=false;
let currentLegaId=null;
let currentLegaMeta=null;
let currentUser=null; // Firebase Auth user
const GIORNATE_FALLBACK = { 1:"G1", 2:"G2", 3:"G3", 4:"G4", 5:"G5", 6:"G6", 7:"G7", 8:"G8", 9:"PO-A", 10:"PO-R", 11:"R16-A", 12:"R16-R", 13:"QF-A", 14:"QF-R", 15:"SF-A", 16:"SF-R", 17:"F" };
const GIORNATE = new Proxy({}, { get: (_,id) => (typeof t==="function" ? t("giornate."+id) : null) || GIORNATE_FALLBACK[id] || "G"+id });
...
async function joinLegaById(id, closeFn, auto){
  ...
  if(!meta){
    ...
    if(auto){
      if(localStorage.getItem("ucl_lastLega")===id) localStorage.removeItem("ucl_lastLega");
  ...
  import("https://www.gstatic.com/firebasejs/10.12.0/firebase-auth.js").then(({onAuthStateChanged})=>{
    onAuthStateChanged(window._fbAuth, user=>{
      ...
      if(!currentLegaId){
        ...
        const target = legaParam || localStorage.getItem("ucl_lastLega");
...
function _renderGiocTabellone() {
  ...
  // UCL non ha gironi — iterazione flat su tutti i club presenti nel db
...
// ── LA MIA SQUADRA ───────────────────────────────────────────
const ROSA_REQUISITI = { P:4, D:12, C:12, A:8 };
const ROSA_TOTALE    = 36; // 4+12+12+8 — 1 giocatore per club UCL (36 club)
const DEADLINE_ISO   = "2026-09-16T18:00:00Z"; // prima della prima partita UCL G1 (≈ 16 set ore 20 CEST)
const FINALE_ISO     = "2027-05-30T06:00:00Z"; // dopo la finale UCL (≈ 29 maggio 2027 ore 21 CEST)
